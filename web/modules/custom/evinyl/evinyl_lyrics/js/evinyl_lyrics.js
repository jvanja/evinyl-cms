  ...
  Drupal.behaviors.evinylLyricsJS = {
    attach: function (context) {
      ...
      function processAlbumHtml(elem) {
        ...
        const addLyricsButton = document.querySelector('#field-a-side-songs-values h4')
      ...
      function getLyricsFromMusicMatch(elem) {
        ...
        if (artistName.length > 1) {
          songsNamesFields.forEach(async (field) => {
            const songIdArr = field.classList.value.split(' ').filter((cl) => cl.indexOf('paragraph-id--') == 0)
            const songId = songIdArr[0].replace('paragraph-id--', '')

            const songName = field.querySelector('.field--name-field-song-name').innerText
            ...
            if (lyrics.message.header.status_code === 200) {
              updateTrackWithLyrics(songId, lyrics.message.body.lyrics.lyrics_body)
